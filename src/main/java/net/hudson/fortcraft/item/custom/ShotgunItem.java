            ...
            if(firing>=fire_rate) {
                if (!itemstack.isEmpty() || flag) {
                    ...
                    if (!((double) f < 0.1D)) {
                        ...
                        if (!pLevel.isClientSide) {
                            int Bulletnum = 10;
                            ArrayList<AbstractArrow> bulletList = new ArrayList<>();
                            for(int j = 0;j<Bulletnum;j++){
                                ArrowItem arrowitem = (ArrowItem) (itemstack.getItem() instanceof ArrowItem ? itemstack.getItem() : Items.ARROW);
                                AbstractArrow abstractarrow = arrowitem.createArrow(pLevel, itemstack, pPlayer);
                                abstractarrow = customArrow(abstractarrow);
                                bulletList.add(abstractarrow);

                            }

                            f *= 3.0F;
                            for(AbstractArrow bullet: bulletList){
                                bullet.setBaseDamage(8);
                                bullet.shootFromRotation(pPlayer, pPlayer.getXRot(), pPlayer.getYRot(), 0.0F, 1.5F, 10.0F);
